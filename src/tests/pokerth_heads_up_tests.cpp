...
TEST(HeadsUp_BugVerification, TestFlopBetDisplayIssue)
{
    ...
    // The user reports:
    // - GUI shows bet as 40 (this is BB's total bet of $40, which might be correct)
    // - SB button shows "Call for 0" (WRONG - should be "Call for 20")
    ...
    // If the GUI shows "Call for 0", there might be a bug in how getMySet() is returned
