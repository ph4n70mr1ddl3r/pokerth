...
ChatTools::ChatTools(QLineEdit* l, ConfigFile *c, ChatType ct, QTextBrowser *b, QStandardItemModel *m, gameLobbyDialogImpl *lo) : QObject(lo), nickAutoCompletitionCounter(0), myLineEdit(l), myNickListModel(m), myNickStringList(), myTextBrowser(b), myChatType(ct), myConfig(c), myNick(""), myLobby(lo)
{
